...
/// Dedicated lawyer chat screen opened after a successful payment.
/// AppBar shows the lawyer's name/avatar instead of the JudisAI logo.
/// On init, sends a context-priming message so the AI responds in the
/// lawyer's persona, complete with case summary from the provider.
...
class _LawyerChatScreenState extends State<LawyerChatScreen> {
  ...
  Future<void> _sendInitialContext() async {
    ...
    try {
      ...
      // Fallback greeting if backend unavailable
      if (mounted) {
        setState(() {
          _messages.add(ChatMessage(
            role: 'assistant',
    ...
    return Scaffold(
      body: BlobBackground(
        child: SafeArea(
          child: Column(
            children: [
              // ── Header (lawyer, not JudisAI logo) ────────────────────────
              Padding(
              ...
              if (_messages.any((m) => m.content.contains('Drafting your complaint letter')))
                Padding(
